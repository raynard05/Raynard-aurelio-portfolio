...
export default function skillsSection() {
  ...
  /* === GSAP ANIMATION LOGIC - DRAMATIC POP-OUT === */
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      cardsRef.current.forEach((card, i) => {
        ...
        // DRAMATIC 3D MODEL POP-OUT (r3f-wrapper)
        const r3f = card.querySelector(".r3f-wrapper");
        if (r3f) {
          // Create explosive pop-out effect
          gsap.fromTo(r3f,
            {
              z: 0,
              scale: 1,
            ...
            {
              z: 2000,           // EXPLOSIVE forward movement
              scale: 5,          // MASSIVE scale increase
              rotationY: col === 0 ? -180 : col === 2 ? 180 : 360, // Dramatic spin
              rotationX: 45,     // Tilt forward
              y: -200,           // Lift up dramatically
              ease: "power3.out",
              scrollTrigger: {
                trigger: card,
                start: "top 80%",
        ...
        // CARD WRAPPER ANIMATIONS (Directional Flow)
        let cardAnimation = {};

        if (col === 0) {
          // Left Column → Explode Left
          cardAnimation = {
            x: -800,
            rotationY: -90,
            opacity: 0,
          ...
          // Middle Column → Zoom Past Camera
          cardAnimation = {
            z: 2500,
            scale: 3,
            opacity: 0,
